...
Please run `npm run lint` and `npm run test` before every commit possible.

You can start a local development using `npm run dev`.
As Vite is using caching of sources, changes in the imported packages from this repository may not propagate.
