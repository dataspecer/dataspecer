...
import { extractPartOfRepositoryURL, GitCredentials, GitProvider, GitProviderFactory } from "../git-providers.ts";
...
export const commitPackageToGit = async (
  ...
  // TODO RadStr: Takze PackageExporter muze byt klidne interface s ruznymi implementacemi
  const exporter = new PackageExporterToFileSystem(resourceModel);    // We used exportPackageResource in the exportPackageResource
  await exporter.doExport(iri, directoryWithContent + "/");
...
export class PackageImporterToFileSystem {
