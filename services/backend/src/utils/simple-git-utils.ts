...
/**
 * Performs git clone on {@link git}. The parameters are set up in such a way that only the commit objects are fetched,
 ...
 */
export async function gitCloneOnlyCommits(
    git: SimpleGit,
    ...
    try {
        ...
        console.info("Error on clone", err);        // TODO RadStr: Debug print
        try {
            await git.init();
            await git.pull(repositoryURL);       // If it fails here, it failed completely
        }
        catch (err2) {
            console.info("I am removing directory", err2);      // TODO RadStr: Debug print
