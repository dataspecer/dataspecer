...
export class GitHubProvider extends GitProviderBase {
  ...
  async revokePAT(personalAccessToken: string): Promise<FetchResponse> {
    ...
    const url = `https://api.github.com/applications/${configuration.authConfiguration.gitAuthClientId}/token`;
    ...
    const response = await httpFetch(url, {
      ...
      headers: {
        "Accept": "application/vnd.github+json",
        "X-GitHub-Api-Version": "2022-11-28",
        "Authorization": "Basic " + Buffer.from(`${configuration.authConfiguration.gitAuthClientId}:${configuration.authConfiguration.gitAuthClientSecret}`).toString("base64"),
        "Content-Type": "application/json"
      },
      body: JSON.stringify({
