  ...
  async revokePAT(personalAccessToken: string): Promise<FetchResponse> {
    ...
    const response = await httpFetch(url, {
      ...
      headers: {
        "Accept": "application/vnd.github+json",
        "X-GitHub-Api-Version": "2022-11-28",
        "Authorization": "Basic " + Buffer.from(`${configuration.authConfiguration.gitHubAuthClientId}:${configuration.authConfiguration.gitHubAuthClientSecret}`).toString("base64"),
        "Content-Type": "application/json"
      },
      body: JSON.stringify({
