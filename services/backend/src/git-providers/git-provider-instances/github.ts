...
import { GIT_RAD_STR_BOT_EMAIL, GIT_RAD_STR_BOT_SSH_ID, GIT_RAD_STR_BOT_USERNAME, GITHUB_AUTH_CLIENT_ID, GITHUB_AUTH_CLIENT_SECRET, GITHUB_RAD_STR_BOT_ABSOLUTE_CONTROL_TOKEN } from "../../git-never-commit.ts";
...
export class GitHubProvider extends GitProviderBase {
  ...
  async revokePAT(personalAccessToken: string): Promise<FetchResponse> {
    ...
    const url = `https://api.github.com/applications/${GITHUB_AUTH_CLIENT_ID}/token`;
    ...
    const response = await httpFetch(url, {
      ...
      headers: {
        "Accept": "application/vnd.github+json",
        "X-GitHub-Api-Version": "2022-11-28",
        "Authorization": "Basic " + Buffer.from(`${GITHUB_AUTH_CLIENT_ID}:${GITHUB_AUTH_CLIENT_SECRET}`).toString("base64"),
        "Content-Type": "application/json"
      },
      body: JSON.stringify({
