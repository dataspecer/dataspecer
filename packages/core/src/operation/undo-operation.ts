 ...
 * model(s) it targets. The history then behaves as if the cancelled
 * transaction's operations never happened: the state is the state before the
 * cancelled transaction with all later non-cancelled operations replayed on top
 * of it.
 *
...
export function filterCancelledTransactions<T extends Transaction>(transactions: readonly T[]): T[] {
  ...
  for (const transaction of transactions) {
    ...
    const operations = transaction.operations.filter(
      ({ modelId, operation }) => !isUndoOperation(operation) && !cancelledModels?.has(modelId),
    );
