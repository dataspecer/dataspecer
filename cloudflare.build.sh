...
CONTROLLED_VOCAB_MANAGER_BASE_PATH="$BASE_PATH$CONTROLLED_VOCAB_MANAGER"
CONTROLLED_VOCAB_MANAGER_URL="$CONTROLLED_VOCAB_MANAGER_BASE_PATH"
...
printf "VITE_MANAGER=$MANAGER_URL\nVITE_BACKEND=$BACKEND\n" > applications/controlled-vocab-manager/.env.local
...
if [ -n "$DO_BUILD_BACKEND" ]; then
  ...
  npx turbo run build  --cache=local: --concurrency 100% --filter=data-specification-editor --filter=conceptual-model-editor --filter=controlled-vocab-manager --filter=manager --filter=api-specification --filter=backend^...
  (cd services/backend && npx npm run build-pack)
else
  npx turbo run build  --cache=local: --concurrency 100% --filter=data-specification-editor --filter=conceptual-model-editor --filter=controlled-vocab-manager --filter=manager --filter=api-specification
fi
...
cp -r applications/controlled-vocab-manager/dist/* .dist$CONTROLLED_VOCAB_MANAGER
